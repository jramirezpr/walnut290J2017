...
//
// g++ RichExtr1DandStoppingMainMod.cpp -std=c++11 -I../../290J_2015 -lfftw3 -o Rich1DTestDebu.exe
//
...
void heatEqStep1D(double tau, UCLAQ::fftw3_sin1d& DFT, UCLAQ::GridFunction1d&  uIn, UCLAQ::GridFunction1d&  uOut, double alpha = -1.0){//1D heatequation step
	double minuseigkx = 0;
...
string nameOfFile=to_string(tau);
    ...
    //long zPanels  = 100;
if(xPanels==150){
nameOfFile="RichardsonResults1Dim150x150x150pt"+nameOfFile+".txt";//1DIM
fout.open(nameOfFile.c_str());
cout<<"FILE 150OPEN";

}
if(xPanels==100){
nameOfFile="RichardsonResults1Dim100x100x100pt"+nameOfFile+".txt";//1DIM
fout.open(nameOfFile.c_str());
cout<<"FILE OPEN";

}
if(xPanels==50){
nameOfFile="RichardsonResults1Dim50x50x50"+nameOfFile+".txt";//1DIM
fout.open(nameOfFile.c_str());
cout<<"FILE 50 OPEN";
...
UBeforeInnLoop=Uinit;
	//while(((jInner<innMaxIt)&&(inRelerr>innTol))||(jInner<=1))// or jinner>3
        while((inRelerr>tau*tau*innTol)||(jInner<=1))
		{
				...
				Richardson1D(tau,DFT,Uinit,Uout,nuclearPotential1D);//done tests with this one
