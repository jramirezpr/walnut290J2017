...
using namespace std;
//
// fftw_discreteSin2dSolveTest.cpp
//
// A test code that tests the use of FFTW to create a solution of the 2d
...
// alpha*Delta_h  = f
//
// where Delta_h is the standard five-point discretization of the Laplace operator
// and the exact solution u is the function (1-(x^2 + y^2))^q scaled and centered so that
...
//
// g++ fftw_discreteSin2dSolveTest.cpp -std=c++11 -I../../290J_2015 -lfftw3 -o fftw_discreteSin2dSolveTest.exe
//
// Alternately, if one is using MakeScripts, the build command executed from within 290J_Samples/FFTW_DiscreteSinSolveTest is
//
// make -f fftw_discreteSin2dSolveTest.mk release
//
...
// Use a relative path to LaplaceOp2d so that additions to the
...
//   how to scale the transforms of derivatives appropriately
    ...
    // Checking evaluation of the alpha*Delta_h
    ...
    for(long ky = 1; ky <= yPanels-1; ky++)
    {
    	...
    	gamma_kx_ky = alpha*( (-4.0/(hx*hx))*sin(kx*pi*(hx/2.0)/LX)*sin(kx*pi*(hx/2.0)/LX)
    		                + (-4.0/(hy*hy))*sin(ky*pi*(hy/2.0)/LY)*sin(ky*pi*(hy/2.0)/LY) );


    	uTransform(kx-1,ky-1) *= gamma_kx_ky;
    }}
    ...
    for(long ky = 1; ky <= yPanels-1; ky++)
    {
    	...
    	fTransform(kx-1,ky-1) *= 1.0/gamma_kx_ky;
    }}
    ...
    printf("Error in the evaluation of the solution to alpha*Delta_h = f : %10.5e \n",uError.norm2());
