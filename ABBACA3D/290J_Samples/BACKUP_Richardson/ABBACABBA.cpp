...
// ABBACABBA.cpp
//
// Solve Schroedingers equation with abbacabba procedure.
//
...
//
// g++ ABBACABBA.cpp -std=c++11 -I../../290J_2015 -lfftw3 -o ABBACABBA.exe
//
...
for(long kx = 1; kx <= potential.xPanels-1; kx++)
    ...
    for(long kz = 1; kz <= potential.zPanels-1; kz++)
    {
    	if(abs(uOut.Values(kx,ky,kz))<max)
	{max=abs(uOut.Values(kx,ky,kz));
	xm=kx; ym=ky;zm=kz;
}
    }}}
...
void ABBACABBAstep(double tau, int xPanels,double xMin,double xMax, int yPanels,double yMin,double yMax, int zPanels,double zMin,double zMax, UCLAQ::fftw3_sin3d& DFT,UCLAQ::GridFunction3d&  uIn,    UCLAQ::GridFunction3d&  uOut,UCLAQ::GridFunction3d& potential,double LX,double LY,double LZ){
...
UCLAQ::GridFunction3d  AUX(xPanels,xMin,xMax,yPanels,yMin,yMax,zPanels,zMin,zMax); 
  ...
  AUX2.setToValue(0.0);
uOut.setToValue(0.0);
ABBACABBAstep(tau*x_1,xPanels,xMin,xMax,yPanels,yMin,yMax,zPanels,zMin,zMax,DFT,uIn,AUX,potential,LX,LY,LZ);
ABBACABBAstep(tau*x_0,xPanels,xMin,xMax,yPanels,yMin,yMax,zPanels,zMin,zMax,DFT,AUX,AUX2,potential,LX,LY,LZ);
ABBACABBAstep(tau*x_1,xPanels,xMin,xMax,yPanels,yMin,yMax,zPanels,zMin,zMax,DFT,AUX2,uOut,potential,LX,LY,LZ);
uOut/=uOut.norm2();
}
...
string nameOfFile=to_string(tau);
    ...
    long zPanels  = 100;
if(xPanels==150){
nameOfFile+="Results150x150x150.txt";
fout.open(nameOfFile.c_str());
cout<<"FILE 150OPEN";

}
if(xPanels==100){
nameOfFile+="Results100x100x100.txt";
fout.open(nameOfFile.c_str());
cout<<"FILE OPEN";

}
if(xPanels==50){
nameOfFile+="Results50x50x50.txt";
fout.open(nameOfFile.c_str());
cout<<"FILE 50 OPEN";
...
cout<<"ORIGINAL VALUE"<<lambdaAUX<<endl;
		heatEqStep(tau/2.0,xPanels,yPanels,zPanels,DFT,Uinit,Uout,LX,LY,LZ);
uTemp2=Uout;
uTemp2/=uTemp2.norm2();
uTemp3=uTemp2;
ShrOP3d.apply(uTemp2);
lambdaAUX=uTemp2.scaledDot(uTemp3)/(uTemp3.scaledDot(uTemp3));
cout<<"heat step"<<lambdaAUX<<endl;
  UCLAQ::GridFunction3d  AUX(xPanels,xMin,xMax,yPanels,yMin,yMax,zPanels,zMin,zMax);
cout<<"min of potentiaql"<<nuclearPotential.min()<<endl;
	PotentialStep(tau/2.0,xPanels,yPanels,zPanels,Uout,AUX,nuclearPotential); 
uTemp2=AUX;
uTemp2/=uTemp2.norm2();
uTemp3=uTemp2;
ShrOP3d.apply(uTemp2);
lambdaAUX=uTemp2.scaledDot(uTemp3)/(uTemp3.scaledDot(uTemp3));
cout<<"Potential Step"<<lambdaAUX<<endl;
	PotentialStep(tau,xPanels,yPanels,zPanels,AUX,Uout,nuclearPotential); 
uTemp2=Uout;
uTemp2/=uTemp2.norm2();
uTemp3=uTemp2;
ShrOP3d.apply(uTemp2);
...
heatEqStep(tau,xPanels,yPanels,zPanels,DFT,Uout,AUX,LX,LY,LZ);
uTemp2=AUX;
uTemp2/=uTemp2.norm2();
uTemp3=uTemp2;
ShrOP3d.apply(uTemp2);
...
ABBACABBAstep(tau,xPanels,xMin,xMax,yPanels,yMin,yMax,zPanels,zMin,zMax,DFT,Uinit,Uout,nuclearPotential,LX,LY,LZ);
